...
export default class SafariGrid extends Component<{}> {
  constructor(props) {
    super(props);
  ...
  makeLevel(c) {
    let allNumbers = initAllNumbers();
    let targets = allNumbers.filter(e => e.value == c);
    let theRest = allNumbers.filter(e => e.value != c);
    let shuffleTheRest = shuffleArray(theRest, []);
    let targetsLast = [...shuffleTheRest, ...targets];
    let firstBoard = targetsLast.slice(35);
    let shuffled = shuffleArray(firstBoard, []);
    console.log("targets", shuffled.length);
    return [...shuffled, ...shuffleTheRest];
  }
  ...
  // Animates the values in the tile data source based on their index in the array.
  animateValuesToLocationsWaterfalStyle(callback) {
    ...
    this.state.tileDataSource.map((row, i) => {
      ...
      row.map((elem, j) => {
        ...
        Animated.sequence([
          ...
          if (calledBack == false && callback) {
            console.log("this should only execute once");
            callback();
            calledBack = true;
          }
        });
  ...
  animateValuesToLocationsLinearStyle(callback) {
    let calledBack = false;
    this.state.tileDataSource.map((row, i) => {
      row.map((elem, j) => {
        elem.scale.setValue(0);
        Animated.parallel([
          Animated.spring(elem.location, {
            toValue: { x: TILE_WIDTH * i, y: TILE_WIDTH * j },
            friction: 4,
  ...
  recolorAllMatches() {
    console.log("how many times is this getting called for christ sakeon");
    this.cardCue = this.makeLevel(this.currentLevelIndex + 1);
    console.log("card cue length", this.cardCue.length);
    ...
    this.state.tileDataSource.forEach(row => {
      row.forEach(e =>
        Animated.timing(e.scale, {
          ...
          if (!calledBack && callback) {
            console.log("how man fucking times is this getting called?");
            calledBack = true;
            callback();
          }
        })
  ...
  onTouch(indices) {
    ...
    if (value == this.currentLevelIndex + 1) {
      ...
      Animated.sequence([
        ...
        if (this.numbersFound.length == 6) {
          this.currentLevelIndex += 1;
          this.props.nextPrompt(this.currentLevelIndex);
          this.shrinkGrid(() => {
  ...
  renderTiles() {
    console.log("NEW RENDER TILES BEING CALLED!!!");
    if (this.firstLoad) {
      ...
      console.log("no londer first load");
      let components = [];
      // This creates the array of Tile components that is stored as a state variable.
      this.state.tileDataSource.map((row, i) => {
        let rows = row.map((e, j) => {
  ...
  resetGrid() {
    this.recolorAllMatches();
    this.animateValuesToLocationsLinearStyle();
    this.numbersFound = [];
  }
