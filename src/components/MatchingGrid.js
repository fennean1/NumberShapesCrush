...
class TileData {
  ...
  setAsset(obj) {
    console.log("incoming asset object type", obj.type);
    this.assetObj = obj;
...
export default class Swappables extends Component<{}> {
  ...
  renderTiles() {
    if (this.firstLoad) {
      ...
      this.visualCardsWithProps = getLevel(this.props.currentLevel);
      console.log(
        "visual cards with props length",
  ...
  initializeDataSource() {
    ...
    var tileData = keys.map((row, i) => {
      let dataRows = row.map((key, j) => {
        let randInt = getRandomInt(this.visualCardsWithProps.length - 1);
        let newAsset = this.visualCardsWithProps[randInt];
  ...
  recolorMatches(neighbors) {
    neighbors.map(e => {
      ...
      let newAsset = this.visualCardsWithProps[randInt];
      console.log(
        "visualCardsWithProps length prior to if block",
        this.visualCardsWithProps.length
      );
      console.log("curent asset type", currentAssetObject.type);
      console.log("Types.NUMERAL", Types.NUMERAL);
      /*
      if (currentAssetObject.type != Types.NUMERAL) {
        console.log("pushing back");
        this.visualCardsWithProps.push(currentAssetObject);
      }
      */
      console.log(
      ...
      console.log("visualCardsWithProps", this.visualCardsWithProps);
  ...
  onTouch(indices) {
    ...
    if (!this.cancelTouches) {
      ...
      this.state.tileDataSource[i][j].selected = !this.state.tileDataSource[i][
        j
      ].selected;
      this.setState({
        tileDataSource: this.state.tileDataSource
      ...
      this.potentialMatchTiles.push(this.state.tileDataSource[i][j]);
      ...
      if (this.potentialMatch.length == 2) {
        if (this.checkMatch(this.selectedIndexes)) {
          ...
          this.potentialMatchTiles[0].selected = false;
          this.potentialMatchTiles[1].selected = false;

          this.potentialMatchTiles[0].setAsset(
            this.potentialMatchTiles[0].assetObj.numeral
          );
          this.potentialMatchTiles[1].setAsset(
            this.potentialMatchTiles[1].assetObj.numeral
          );

          const animateCorrectMatch = Animated.parallel([
            Animated.timing(this.potentialMatchTiles[0].scale, {
              toValue: 1.25,
              duration: 250,
              useNativeDriver: true
            }),
            Animated.timing(this.potentialMatchTiles[1].scale, {
              toValue: 1.25,
              duration: 250,
              useNativeDriver: true
            })
